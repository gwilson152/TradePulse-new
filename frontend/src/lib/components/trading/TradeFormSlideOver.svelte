<script lang="ts">
	...
	function handleSubmit() {
		...
		if (!formData.symbol || !formData.quantity || !formData.entryPrice) {
			error = 'Please fill in all required fields in Trade and Entry tabs';
			activeTab = 'trade';
			return;
		}
...
>
	...
	{#if activeTab === 'exit'}
		<FormSection title="Exit Details (Optional)" icon="mdi:logout">
			<div class="grid grid-cols-1 gap-4">
				<Input
					label="Exit Price"
					type="number"
			...
			<HelpText
				type="info"
				text="Leave exit details blank if the position is still open. You can add exits later when you close or partially close the position."
			/>
		</FormSection>
	{/if}
